  ...
  return (
    <div className={`py-4 min-h-screen ${styles.section}`}>
      ...
      <div className="w-full flex items-center justify-between pt-6">
        ...
        <h5 className="text-[#00000084]">
          Được đặt ngày: <span>{data?.createdAt?.slice(0, 10)}</span>
        </h5>
      </div>
      ...
      <div className="w-full 800px:flex items-center">
        ...
        <div className="w-full 800px:w-[40%]">
          ...
          <h4>
            Trạng thái:{" "}
            {data?.paymentInfo?.status ? data?.paymentInfo?.status : "Chưa hoàn thành"}
          </h4>
        </div>
      </div>
      <br />
      ...
      {data?.status !== "Đang xử lý tiền hoàn lại" && data?.status !== "Hoàn tiền thành công" && (
        <select
          value={status}
